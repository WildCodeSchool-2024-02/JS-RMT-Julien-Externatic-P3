...
class TechnologyOfferRepository extends AbstractRepository {
  constructor() {
    // Call the constructor of the parent class (AbstractRepository)
    // and pass the table name "technology" as configuration
    super({ table: "technology_offer" });
  }
  ...
  async create(techno) {
    ...
    const [result] = await this.database.query(
      `insert ignore into ${this.table} (technology_id, offer_id) values (?, ?)`,
      [techno.technology_id, techno.offer_id]
    );
  ...
  async delete(techno) {
    ...
    const [result] = await this.database.query(
      `DELETE FROM ${this.table} WHERE technology_id = ? AND offer_id = ?`,
      [techno.technologyId, techno.offerId]
    );
...
module.exports = TechnologyOfferRepository;
